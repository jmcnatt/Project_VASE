 ...
 */
class DeployThread extends ThreadExt
{
		...
		if (!team.equals("< Not Set >"))
		{
			try
			{
				...
				if (folder == null)
				{
					...
					LOG.write("Folder created for " + team, true);
					for (Object each : engine.rootDir.getChildEntity())
					{
						if (each instanceof Folder)
						{
		...
		try
		{
			...
			for (VirtualEthernetCard each : nics)
			{
				...
				if (network != null)
				{
					...
					{
						LOG.write(vm.getName() + " changed to network \"" + network.getName() + "\"", true);
					}
					
					else
					{
						LOG.write(vm.getName() + " could not be changed to \"" + network.getName() + "\"", true);
					}
				}
				
				else
				{
					LOG.write("Error in changing network label.  The network does not exist", true);
				}
			}
		}
	...
	public void run()
	{
		for (DeployedVirtualMachine newVM : deployment)
		{
			...
			try
			{
				...
				//Step 1 - Deploy
				LOG.write("Creating " + guestName);
				Task task = vm.cloneVM_Task(getLocation(newVM.getTeam()), guestName, spec);
				
				if (task.waitForTask() == Task.SUCCESS)
				{
					LOG.write(guestName + " created successfully");
				}
				
				else
				{
					LOG.write("Error: Clone of Template \"" + templateName + "\" was not successful.\n" +
				...
				if (cloneSuccessful)
				{
					...
					TEAMS.put(newVM.getVmName(), newVM.getTeam());
				}
				
				LOG.write(newVM.getVmName() + " deployed successfully");
			}
